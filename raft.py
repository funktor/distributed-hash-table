...
class Raft:
    def __init__(self, ip, port, partitions):
        ...
        self.commit_log = CommitLog(
            file=f"commit-log-{self.ip}-{self.port}.txt")
        self.partitions = eval(partitions)
        self.conns = [[None]*len(self.partitions[i])
        ...
        self.state = 'FOLLOWER' if len(
            self.partitions[self.cluster_index]) > 1 else 'LEADER'
        self.leader_id = -1
        self.commit_index = 0
    ...
    def listen_to_clients(self):
        ...
        while True:
            try:
                ...
                my_thread = Thread(
                    target=self.process_request, args=(client_socket,))
